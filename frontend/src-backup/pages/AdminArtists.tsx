...
const AdminArtists = () => {
    ...
    const [form, setForm] = useState({ name: '', bio: '', image: null as File | null });
    ...
    const notify = (text: string) => { setMsg(text); setTimeout(() => setMsg(null), 3000); };
    ...
    const handleDelete = async (id: number) => {
        ...
        if (res.ok) { notify('Artist deleted.'); load(); }
    };

    return (
        <div className="admin-container animate-enter">
            ...
            <div className="admin-split-layout">
                ...
                <div className="glass-panel admin-form-card">
                    ...
                    <form onSubmit={handleSubmit} className="artist-form">
                        ...
                        <div style={{ display: 'flex', gap: 10 }}>
                            ...
                            {editId && (
                                <button type="button" className="cancel-btn" onClick={() => { setEditId(null); setForm({ name: '', bio: '', image: null }); }}>
                                    Cancel
                                </button>
                            )}
                        </div>
                ...
                <div className="artists-list-admin">
                    {artists.length === 0 ? (
                        <div className="chart-empty glass-panel">No artists yet. Add one!</div>
                    ) : artists.map(a => (
                        <div key={a.id} className="artist-list-item glass-panel">
                            {a.imagePath ? (
                                <img
                                    src={a.imagePath.startsWith('http') ? a.imagePath : `${API_URL}${a.imagePath}`}
                                    alt={a.name}
                                    className="artist-list-thumb"
                                />
                            ) : (
                                <div className="artist-list-thumb-placeholder"><Mic2 size={22} /></div>
                            )}
