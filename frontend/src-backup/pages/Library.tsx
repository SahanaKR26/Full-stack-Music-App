...
type Tab = 'liked' | 'recent' | 'recommended';
...
const Library = () => {
    ...
    const getAlbumArt = (s: Song) =>
        s.albumArtPath?.startsWith('http') ? s.albumArtPath : s.albumArtPath ? `${API_URL}${s.albumArtPath}` : null;
    ...
    const SongRow = ({ s }: { s: Song }) => (
        <div className="song-row" onClick={() => playSong(s, songs)}>
            <div className="song-row-art">
                {getAlbumArt(s) ? (
                    <img src={getAlbumArt(s)!} alt="" />
                ) : (
                    <div className="song-row-art-placeholder">♪</div>
                )}
    ...
    return (
        <div className="library-container animate-enter">
            ...
            {loading ? (
                ...
                <div className="songs-list glass-panel">
