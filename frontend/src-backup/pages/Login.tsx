...
const Login = () => {
    ...
    const handleLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            login(data.token, data.username, data.userId, data.role, data.subscriptionPlan);
            ...
            <div className="w-full lg:w-1/2 flex flex-col justify-center px-6 py-12 lg:px-24 bg-background-light dark:bg-background-dark min-h-screen">
                <div className="max-w-md w-full mx-auto flex-1 flex flex-col justify-center">
                    ...
                    <div className="flex p-1 mb-8 bg-slate-200 dark:bg-primary/10 rounded-xl">
                        <button className="flex-1 py-2 text-sm font-semibold rounded-lg bg-white dark:bg-primary text-slate-900 dark:text-white shadow-sm transition-colors cursor-default">Login</button>
                        <Link to="/register" className="flex-1 py-2 text-sm font-semibold text-center text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">Sign Up</Link>
                    </div>
                    ...
                    <form className="space-y-5" onSubmit={handleLogin}>
                        <div>
                            ...
                            <input
                                className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900/50 text-slate-900 dark:text-white focus:ring-2 focus:ring-primary focus:border-transparent transition-all outline-none"
