...
const AdminLayout = () => {
    ...
    return (<div className="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 min-h-screen flex font-display animate-enter">
            {/* Sidebar */}
            <aside className="w-72 border-r border-slate-200 dark:border-primary/10 flex flex-col fixed h-full bg-white dark:bg-background-dark z-50 shadow-xl shadow-slate-200/20 dark:shadow-none">
                ...
                <nav className="flex-1 px-4 py-2 space-y-1 overflow-y-auto">
                    <NavLink to="/admin" end className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${isActive ? 'bg-primary/10 text-primary font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-primary/5 font-medium'}`}>
                        <span className="material-symbols-outlined">dashboard</span>
                        <span>Dashboard Overview</span>
                    </NavLink>
                    <NavLink to="/admin/users" className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${isActive ? 'bg-primary/10 text-primary font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-primary/5 font-medium'}`}>
                        <span className="material-symbols-outlined">group</span>
                        <span>Manage Users</span>
                    </NavLink>
                    <NavLink to="/admin/upload" className={({ isActive }) => `flex items-center gap-3 px-4 py-3 rounded-lg transition-all ${isActive ? 'bg-primary/10 text-primary font-bold' : 'text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-primary/5 font-medium'}`}>
                ...
                <header className="h-20 border-b border-slate-200 dark:border-primary/10 flex items-center justify-between px-8 bg-white/80 dark:bg-background-dark/80 backdrop-blur-md z-40 shrink-0">
                    <div className="flex-1 max-w-xl">
                        <div className="relative group">
                            ...
                            <input className="w-full bg-slate-100 dark:bg-primary/5 border-none rounded-xl py-3 pl-12 pr-4 text-sm focus:ring-2 focus:ring-primary/50 transition-all dark:text-white placeholder:text-slate-400 dark:placeholder:text-primary/40 font-medium" placeholder="Search admin panel..." type="text"/>
                        </div>
                    </div>
