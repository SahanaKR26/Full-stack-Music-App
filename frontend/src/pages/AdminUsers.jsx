...
const AdminUsers = () => {
    ...
    return (<div className="p-8 max-w-7xl mx-auto w-full animate-enter">
            ...
            <div className="bg-white/80 dark:bg-primary/5 rounded-2xl overflow-hidden shadow-2xl border border-slate-200 dark:border-primary/10">
                    ...
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-200 dark:divide-primary/10">
                            {users.length === 0 ? (<tr>
                                    <td colSpan={5} className="px-6 py-10 text-center text-slate-500 font-medium">No users found.</td>
                                </tr>) : (users.map(user => (<tr key={user.id} className={`hover:bg-slate-50/50 dark:hover:bg-primary/5 transition-colors ${user.isBlocked ? 'opacity-50' : ''}`}>
                                        ...
                                        <td className="px-6 py-4">
                                            {user.role === 'Admin' ? (<span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-emerald-500/10 text-emerald-500 border border-emerald-500/20">Admin</span>) : user.role === 'Artist' ? (<span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-amber-500/10 text-amber-500 border border-amber-500/20">Artist</span>) : (<span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-slate-500/10 text-slate-500 border border-slate-500/20">User</span>)}
                                        </td>
                                        <td className="px-6 py-4">
                                            {user.subscriptionPlan === 'Premium' || user.subscriptionPlan === 'Pro' ? (<span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-primary/10 text-primary border border-primary/20">{user.subscriptionPlan}</span>) : (<span className="px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider bg-slate-500/10 text-slate-500 border border-slate-500/20">{user.subscriptionPlan || 'Free'}</span>)}
                                        </td>
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <button onClick={() => handleBlock(user.id)} className="size-8 flex items-center justify-center rounded-lg hover:bg-amber-500/10 hover:text-amber-500 transition-all text-slate-400" title={user.isBlocked ? "Unblock User" : "Suspend User"}>
                                                    <span className="material-symbols-outlined text-[18px]">{user.isBlocked ? 'lock_open' : 'block'}</span>
                                                </button>
                                                <button onClick={() => handleDelete(user.id)} className={`size-8 flex items-center justify-center rounded-lg hover:bg-red-500/10 hover:text-red-500 transition-all ${user.role === 'Admin' ? 'opacity-20 pointer-events-none' : 'text-slate-400'}`} title="Delete User" disabled={user.role === 'Admin'}>
                ...
                {users.length > 0 && (<div className="flex items-center justify-between p-6 border-t border-slate-200 dark:border-primary/10 bg-slate-50/50 dark:bg-transparent">
